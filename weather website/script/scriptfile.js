async function weather(city, id_city, id_temp, id_wind, id_cloud) {
...
btn.addEventListener("click", (e) => {
	e.preventDefault()
	weather(inp.value, "city0", "temp0", "wind0", "cloud0")
})
...
function randomCity() {
	...
	while (true) {
		...
		if (!pickedCities.has(pick)) {
			...
			if (cnt === 1) {
				weather(cities[pick], "city1", "temp1", "wind1", "cloud1");
			}
			else if (cnt === 2) {
				weather(cities[pick], "city2", "temp2", "wind2", "cloud2");
			}
			else if (cnt === 3) {
				weather(cities[pick], "city3", "temp3", "wind3", "cloud3");
			}
...
weather("Barisal", "city0", "temp0", "wind0", "cloud0");
